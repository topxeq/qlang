...
func (arity iCallFn) Exec(stk *Stack, ctx *Context) {
	...
	// fmt.Printf("beginhere2: %#v, %T, %v, %v\n", vfn, vfn, vfn.Type(), vfn.Kind())
	if vfn.Kind() != reflect.Func { // 这不是func，而是Function/其他可调用对象
		// fmt.Printf("beginhere3: %#v, %T, %v, %v\n", vfn, vfn, vfn.Type(), vfn.Kind())
		tfn0 = tfn
		if caller, ok := vfn.Interface().(iCaller); ok {
			...
			if vfnt.IsValid() {
				...
				// fmt.Printf("beginhere9: %#v, %T, %v, %v\n", vfn, vfn, vfn.Type(), vfn.Kind())

				// if vfn.Kind() == reflect.Int {
				// } else {
		...
		// fmt.Printf("beginhere9a: %#v, %T, %v, %v\n", vfn, vfn, vfn.Type(), vfn.Kind())
